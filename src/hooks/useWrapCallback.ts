      ...
      return {
        ...
        execute:
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, {
                    summary: `Wrap ${inputAmount.toSignificant(6)} TARA to WTARA`,
                    type: 'wrap',
                  })
                } catch (error) {
            ...
            : undefined,
        inputError: sufficientBalance ? undefined : t('Insufficient TARA balance'),
      }
    }
          ...
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, { summary: `Unwrap ${inputAmount.toSignificant(6)} WTARA to TARA` })
                } catch (error) {
                  console.error('Could not withdraw', error)
                }
              }
            : undefined,
        inputError: sufficientBalance ? undefined : t('Insufficient WTARA balance'),
      }
    }
