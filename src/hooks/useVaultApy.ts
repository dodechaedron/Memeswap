...
export function useVaultApy({ duration = MAX_LOCK_DURATION }: { duration?: number } = {}) {
  ...
  const { data: totalCakePoolEmissionPerYear } = useSWRImmutable('masterChef-total-cake-pool-emission', async () => {
    ...
    const [[regularFarmsPerBlock], cakePoolInfo, [totalRegularAllocPoint]] = await multicallv2(masterChefAbi, calls)

    const cakePoolSharesInRegularFarms = FixedNumber.from(cakePoolInfo.allocPoint).divUnsafe(
      FixedNumber.from(totalRegularAllocPoint),
    )
    return FixedNumber.from(regularFarmsPerBlock)
      .mulUnsafe(FixedNumber.from(BLOCKS_PER_YEAR))
      .mulUnsafe(cakePoolSharesInRegularFarms)
  })
